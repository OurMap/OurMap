...
This a clone of drag.js, with small modifications.
		...
		// if ordinary element was clicked and left button was released, but element is placed inside unmovable table cell
			...
			if (table < tables.length)
				...
				target_cell = target_table.rows[row].cells[cell];
                                ...
                                var kw_value = obj.innerHTML.trim();
                                if ( myTargetCat != mySourceCat )
                                    updateKeyword( kw_value, myTargetCat );
                                
                                //alert('From: ' + mySourceCat + ' to ' + myTargetCat + ' Obj:' + kw_value );
...
function updateKeyword( kwId, catId ) {
         ...
         var parameters =  'kwValue=' + kwId + '&catname=' + catId ;

         //alert( parameters );


         makePOSTRequest( http_request, 'editsessionkeyword.do' , parameters, function() {
         if (http_request.readyState == 4 && http_request.status == 200 )
         {
           ...
           {
              document.location = 'newmap.do?status=6.5' ;
              //orgCatsSession();
           }
